...
export interface SellerScheduleEditorProps {
  sellerId: string;
  sellerName?: string;
  onSaved?: () => void;
  onRemoved?: () => void;
...
export function SellerScheduleEditor({
  sellerId,
  sellerName,
  onSaved,
  onRemoved,
}: SellerScheduleEditorProps) {
  ...
  useEffect(() => {
    ...
    setLateToleranceMinutes(
      currentSchedule.lateToleranceMinutes !== null &&
        currentSchedule.lateToleranceMinutes !== undefined
        ? String(currentSchedule.lateToleranceMinutes)
        : '',
    );
    setEntryToleranceMinutes(
      currentSchedule.entryToleranceMinutes !== null &&
        currentSchedule.entryToleranceMinutes !== undefined
  ...
  if (loading) {
    return (
      <div className="space-y-4">
        {sellerName ? <Skeleton className="h-6 w-1/2" /> : null}
    ...
    <div className="space-y-4">
      {sellerName ? (
        <p className="text-sm text-muted-foreground">
          Configure os dias e horários individuais de <strong>{sellerName}</strong>. Se não houver
          nada configurado, este vendedor usará a jornada da empresa.
        </p>
      ) : (
        <p className="text-sm text-muted-foreground">
          Configure os dias e horários individuais. Se não houver nada configurado, este vendedor
          usará a jornada da empresa.
        </p>
      )}
        ...
        <CardContent className="p-4 sm:p-6 space-y-6">
          ...
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <TimeField
                label="Saída Almoço"
                value={defaultLunchOutTime}
                onChange={setDefaultLunchOutTime}
              />
              <TimeField
                label="Volta Almoço"
                value={defaultLunchInTime}
                onChange={setDefaultLunchInTime}
              />
          ...
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label htmlFor={`entryTol-${sellerId}`}>Tolerância de entrada (min)</Label>
                <Input
                  id={`entryTol-${sellerId}`}
                  type="number"
                  min={0}
                  max={60}
              ...
              <div className="space-y-1">
                <Label htmlFor={`lateTol-${sellerId}`}>Tolerância de atraso (min)</Label>
                <Input
                  id={`lateTol-${sellerId}`}
                  type="number"
                  min={0}
