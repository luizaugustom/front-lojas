...
export function CheckoutDialog({ open, onClose, initialClient, onSaleCreated }: CheckoutDialogProps) {
  ...
  // Carregar configuração fiscal (inclui emitOnlyNfe) quando o modal abrir
  const loadFiscalConfigForCheckout = useCallback(async () => {
    if (!isCompany) return;
    try {
  ...
  }, [open, isCompany, loadSellers, checkFiscalConfig, loadCompanyConfig, loadFiscalConfigForCheckout]);
    ...
    shortcuts: [
      {
        ...
        context: ['checkout'],
        preventDefault: false, // Permitir Enter em inputs
      },
      {
        key: 'Escape',
        handler: () => {
  ...
  const handleCustomerCopyConfirm = async () => {
    ...
    try {
      ...
      if (printResult.success) {
        ...
        toast(`Impressão local falhou: ${printResult.error}`, {
          icon: '⚠️',
          duration: 5000,
        });
      }
                  ...
                  <div className="flex-1 relative">
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        // Ao editar o nome após ter selecionado um cliente, limpar seleção (evitar crédito/parcelado do cliente anterior)
