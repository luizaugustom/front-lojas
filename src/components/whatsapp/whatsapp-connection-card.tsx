...
type Props = {
  /** Chamado quando o status "conectado" muda (útil para bloquear cobrança automática) */
...
export function WhatsAppConnectionCard({ onConnectionChange }: Props) {
  ...
  const handleConnectFlow = async () => {
    ...
    setPairingCode(null);
    try {
      const createRes = await whatsappApi.createInstance();
      if (createRes.data?.message && !createRes.data?.created && createRes.data?.message !== 'Instância já existe') {
        if (createRes.data.message.includes('Evolution não configurada')) {
